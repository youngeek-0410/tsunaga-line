...
export default function AchivementAnimation() {
  const windowW = Dimensions.get('window').width;
  const { photoUri, originalMemoryImageUri } = useLocalSearchParams<{
    photoUri: string;
    originalMemoryImageUri: string;
  }>();

  if (!photoUri && !originalMemoryImageUri) {
    router.back();
  }
  return (
    <SafeAreaView
      ...
      >
        ...
        >
          <FilmFrameCard
            alignItems="center"
            justifyContent="space-between"
            source={{ uri: originalMemoryImageUri }}
            styleProps={{ imageWidth: windowW * 0.7 }}
          />
        </MotiView>
        {/* second animation */}
        <MotiView
          from={{ transform: [{ translateY: 500 }], opacity: 0 }}
          animate={{ transform: [{ translateY: 0 }], opacity: 1 }}
          transition={{ type: 'timing', delay: 2000, duration: 1000 }}
        >
          <FilmFrameCard
            alignItems="center"
            justifyContent="space-between"
